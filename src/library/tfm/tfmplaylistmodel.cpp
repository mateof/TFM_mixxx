...
namespace {
const mixxx::Logger kLogger("TFMPlaylistModel");
const int kDownloadTimeoutMs = 60000; // 60 second timeout for downloads
} // anonymous namespace

TFMPlaylistModel::TFMPlaylistModel(QObject* parent,
    ...
    kLogger.info() << "TFM playlist track cache directory:" << m_cacheDir;
}

TFMPlaylistModel::~TFMPlaylistModel() {
}

TrackPointer TFMPlaylistModel::getTrack(const QModelIndex& index) const {
    ...
    if (location.startsWith("http://") || location.startsWith("https://")) {
        // Get the external_id for cache path - more unique than integer id
...
QString TFMPlaylistModel::getTrackLocation(const QModelIndex& index) const {
...
QString TFMPlaylistModel::resolveLocation(const QString& nativeLocation) const {
...
QString TFMPlaylistModel::downloadTrackSync(const QString& url, const QString& destPath, qint64 expectedSize) const {
    ...
    if (expectedSize > 0 && data.size() != expectedSize) {
        ...
        // If we got significantly less data, the file is likely truncated - fail
...
QString TFMPlaylistModel::getFileExtension(const QString& trackName, const QString& url) const {
...
#include "moc_tfmplaylistmodel.cpp"
